...
// ✅ UPDATE: Accept transactionId as a new parameter
const log4js = require("log4js");
const path = require("path");
...
function logTransaction(transactionId, component, user_id, input, output, startTime) {
  const endTime = Date.now();
  const latency = endTime - startTime;
  ...
  const logEntry = {
    timestamp: new Date().toISOString(),
    transaction_id: transactionId, // 👈 LOGGING IT HERE
    user_id: user_id || "anonymous",
    component: component,
    latency_ms: latency,
    token_usage_est: {
